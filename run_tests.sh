...
function usage {
  ...
  echo "                           been added."
  echo "  -n, --nec_portal         test only nec_portal source."
...
included_dirs="openstack_dashboard horizon nec_portal"
...
function process_option {
  ...
  case "$1" in
    ...
    -m|--manage) manage=1;;
    -n|--nec_portal) only_nec_portal=1;;
...
function run_tests_all {
  ...
  echo "Running nec_portal tests"
  export NOSE_XUNIT_FILE=nec_portal/nosetests.xml
  if [ "$NOSE_WITH_HTML_OUTPUT" = '1' ]; then
    export NOSE_HTML_OUT_FILE='nec_portal_nose_results.html'
  fi
  ${command_wrapper} ${coverage_run} $root/manage.py test nec_portal --settings=nec_portal.test.settings $testopts
  # get results of the nec_portal tests
  NEC_PORTAL_RESULT=$?
  ...
  if [ $with_coverage -eq 1 ]; then
    ...
    cov_include=""
    if [ $only_nec_portal -eq 0 ]; then
      cov_include="horizon/*,openstack_dashboard/*,nec_portal/*"
    else
      cov_include="nec_portal/*"
    fi
    cov_omit="/usr*,setup.py,*egg*,.venv/*,nec_portal/test/*"
    ${command_wrapper} python -m coverage.__main__ combine
    ${command_wrapper} python -m coverage.__main__ xml  -i --include=${cov_include} --omit=${cov_omit}
    ${command_wrapper} python -m coverage.__main__ html -i --include=${cov_include} --omit=${cov_omit} -d reports
  fi
  ...
  TEST_RESULT=$(($HORIZON_RESULT || $DASHBOARD_RESULT || $NEC_PORTAL_RESULT || $PEP8_RESULT))
...
function run_makemessages {
  ...
  if [ $check_only -eq 1 ]; then
    ...
    git checkout -- nec_portal/locale/django*.pot
  fi

  exit $(($HORIZON_PY_RESULT || $HORIZON_JS_RESULT || $DASHBOARD_RESULT || $DASHBOARD_JS_RESULT || $NEC_PORTAL_RESULT))
}
...
function run_compilemessages {
  ...
  ${command_wrapper} $root/manage.py compilemessages
  NEC_PORTAL_RESULT=$?
  exit $(($HORIZON_PY_RESULT || $DASHBOARD_RESULT || NEC_PORTAL_RESULT))
}
...
function run_pseudo {
  ...
  do
      ...
      ${command_wrapper} $root/tools/pseudo.py nec_portal/locale/djangojs.pot nec_portal/locale/$lang/LC_MESSAGES/djangojs.po $lang
  done
  exit $?
}
